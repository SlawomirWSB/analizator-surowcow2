...
st.set_page_config(layout="wide", page_title="TERMINAL V134 - FULL SYNC & STATUS")
...
# 2. Pełna Baza Danych - 11 Instrumentów
default_db = [
...
st.markdown('<div class="header-box"><h3>Terminal V134 - Pełna Lista & Status Sync (11.01)</h3></div>', unsafe_allow_html=True)

# 3. Rozbudowany System Informacji po Synchronizacji
if st.button("🔄 SYNCHRONIZUJ I POBIERZ DANE"):
    st.session_state.db = default_db
    st.success("✅ SYNCHRONIZACJA ZAKOŃCZONA: Dane są aktualne (Niedziela, 11 Stycznia).")
    with st.expander("Szczegóły aktualizacji", expanded=True):
        st.write(f"📊 **Liczba instrumentów:** {len(default_db)}")
        st.write("🆕 **Nowe pozycje:** GBP/JPY (SignalProvider), XAU/USD (VasilyTrader - Target 4540)")
        st.write("⚙️ **System RSI:** Zaktualizowano mapy Multi-Timeframe dla wszystkich par.")
    st.rerun()

st.write("---")
col_l, col_r = st.columns([1.3, 2.5])

# --- LEWA STRONA: LISTA SYGNAŁÓW (Wszystkie 11 pozycji) ---
with col_l:
...
# --- PRAWA STRONA: ANALIZA DYNAMICZNA ---
with col_r:
    cur = st.session_state.db[st.session_state.active_idx]
    tf = st.select_slider("Wybierz interwał (Dynamiczne RSI):", options=["1h", "4h", "1D"], value="1D")
