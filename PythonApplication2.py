...
st.set_page_config(layout="wide", page_title="TERMINAL V105 - ULTRA STABLE")
...
# 2. Jednolita Baza Danych (Rozwiązuje KeyError)
...
# --- PANEL GŁÓWNY ---
st.markdown('<div class="header-box"><h3>Terminal V105 - Stabilna Synchronizacja Danych</h3></div>', unsafe_allow_html=True)

if st.button("🔄 WERYFIKUJ I POBIERZ NOWE DANE (Scan 4 kanałów Telegram 11.01)"):
    st.info("Skanowanie w toku... Jutro tutaj pojawią się automatycznie nowe sygnały.")

col_l, col_r = st.columns([1, 1.8])

with col_l:
    st.write("### Lista Sygnałów (Retencja 3D)")
    for idx, s in enumerate(st.session_state.db):
        ...
        with c_an:
            # Poprawna zmiana aktywnego instrumentu
...
with col_r:
    ...
    # Przeliczanie RSI dla wybranego interwału i pary
    current_rsi = round(cur['rsi_base'] + ({"1m": -10, "1h": 5, "1D": 0}.get(tf, 2)), 1)

    # GÓRNE BOKSY - PRZYWRÓCONE WERDYKTY
